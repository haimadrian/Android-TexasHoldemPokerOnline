...
public class HandRankCalculator {
    ...
    private static HandNumericRank calculate(Card[] cards) {
        ...
        if (rank.getHandRank() == Hand.HandRank.NONE) {
        ...
        if (isFlush) {
            if (rank.getHandRank() == Hand.HandRank.STRAIGHT) {
                ...
                if ((cards[4].getCardRank() == Card.CardRank.ACE) && (cards[3].getCardRank() == Card.CardRank.KING)) {
                    rank = new HandNumericRank(Hand.HandRank.ROYAL_FLUSH, rank.getScore());
                } else {
                    rank = new HandNumericRank(Hand.HandRank.STRAIGHT_FLUSH, rank.getScore());
                }
            }
            // There might be FULL_HOUSE or QUADS, which are better than FLUSH. So make sure we do not override them.
            else if (Hand.HandRank.FLUSH.compareTo(rank.getHandRank()) > 0) {
                // If we are here it means that all cards are of the same suit, so sum their rank up
                rank = new HandNumericRank(Hand.HandRank.FLUSH, sumCardsRank(cards));
            }
        }

        // If there was no score, set HIGH_CARD and sum up the cards rank.
        if (rank.getHandRank() == Hand.HandRank.NONE) {
            rank = new HandNumericRank(Hand.HandRank.HIGH_CARD, sumCardsRank(cards));
        }
     ...
     * <ol>
     *     <li>{@link Hand.HandRank#QUADS QUADS}</li>
     *     <li>{@link Hand.HandRank#FULL_HOUSE FULL_HOUSE}</li>
     *     <li>{@link Hand.HandRank#TRIPS TRIPS}</li>
     *     <li>{@link Hand.HandRank#TWO_PAIRS TWO_PAIRS}</li>
     *     <li>{@link Hand.HandRank#PAIR PAIR}</li>
     * </ol>
     * @param cards Set of 5 cards to find pairings in
     * @return A {@link HandNumericRank} containing both {@link Hand.HandRank} and numeric score, based on involved {@link Card.CardRank}.
     */
    private static HandNumericRank evaluatePairings(Card[] cards) {
        ...
        Hand.HandRank handRank = Hand.HandRank.NONE;
        ...
        for (List<Card> cardsOfSameRank : cardRankToCards.values()) {
            ...
            if (cardsOfSameRank.size() == 4) {
                handRank = Hand.HandRank.QUADS;
            }
            // Trips or Full House
            else if (cardsOfSameRank.size() == 3) {
                if (handRank == Hand.HandRank.PAIR) {
                    handRank = Hand.HandRank.FULL_HOUSE;
                } else {
                    handRank = Hand.HandRank.TRIPS;
                }
            }
            // Pair, Two pairs, or Full House.
            else if (cardsOfSameRank.size() == 2) {
                // Make sure we do not override QUADS
                if (handRank != Hand.HandRank.QUADS) {
                    if (handRank == Hand.HandRank.PAIR) {
                        handRank = Hand.HandRank.TWO_PAIRS;
                    } else if (handRank == Hand.HandRank.TRIPS) {
                        handRank = Hand.HandRank.FULL_HOUSE;
                    } else {
                        handRank = Hand.HandRank.PAIR;
                    }
                }
            }
        }
    ...
    private static HandNumericRank evaluateStraight(Card[] cards) {
        ...
        if ((minimumCardRank != Card.CardRank.NONE) && (minimumCardRank.ordinal() <= Card.CardRank.TEN.ordinal())) {
            ...
            if (expectedStraight.equals(actualSet)) {
                handRank = Hand.HandRank.STRAIGHT;
    ...
    public static class HandRankCalculatorResult implements Comparable<HandRankCalculatorResult> {
    ...
    /**
     * A class used to keep {@link Hand.HandRank} and its score as natural number.
     * <p>
    ...
    @AllArgsConstructor
    @NoArgsConstructor // for Jackson
    public static class HandNumericRank implements Comparable<HandNumericRank> {
        /**
         * The rank as enum
         */
